...
<head>
    ...
    <style>
        ...
        .account-menu { position: absolute; top: 100%; right: 0; margin-top: 8px; background: rgba(30, 41, 59, 0.95); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 12px; padding: 8px; min-width: 200px; box-shadow: 0 10px 40px rgba(0, 0, 0, 0.5); z-index: 50; }
        .account-menu-item { padding: 10px 12px; border-radius: 8px; cursor: pointer; transition: all 0.2s; display: flex; align-items: center; gap: 10px; }
...
<body class="h-screen flex flex-col overflow-hidden" x-data="appData()" x-cloak>
    ...
    <div class="overlay" :class="showAccountMenu ? 'show' : ''" @click="showAccountMenu = false"></div>
        ...
        <header class="top-header px-4 py-3 flex items-center justify-between z-20 border-b border-white/10">
            <div class="flex items-center gap-3">
                ...
                <div class="account-dropdown" x-data="{ open: false }">
                    <button @click="open = !open" class="server-selector">
                    ...
                    <div x-show="open" class="account-menu" @click.away="open = false" x-transition>
                        <template x-for="g in guilds" :key="g.id">
                            <button @click="selectGuild(g); open = false" class="account-menu-item w-full text-left">
            ...
            <div class="flex items-center gap-2">
                ...
                <div class="account-dropdown">
                    <button @click="showAccountMenu = !showAccountMenu" class="flex items-center gap-2 p-1 rounded-xl hover:bg-white/10 transition-all">
                        <img :src="userAvatar" class="w-8 h-8 rounded-full border-2 border-indigo-500">
                    </button>
                    <div x-show="showAccountMenu" class="account-menu" @click.away="showAccountMenu = false" x-transition>
                        ...
                        <button @click="showAccountMenu = false; activePanel = 'premium'" class="account-menu-item w-full text-left">
                            <i class="fas fa-crown text-yellow-400"></i>
                            <span class="text-sm text-white">Premium Plans</span>
                        </button>
                        <a href="/logout" class="account-menu-item w-full text-left text-red-400 hover:bg-red-500/10">
            ...
            <div x-show="activePanel === 'home' && selectedGuild" class="space-y-6">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        <button @click="showAccountMenu = false; activePanel = 'premium'" class="glass p-4 flex flex-col items-center gap-2 border-t-4 border-t-purple-500">
    ...
    <script>
        function appData() {
            return {
                ...
                activePanel: 'home', showMenu: false, showAccountMenu: false, activeCategory: 'moderation', expandedCategory: 'moderation',
                showPayment: false,
                selectedPlan: { 
                    tier: 'silver', 
                    duration: 'monthly', 

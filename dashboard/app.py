...
<head>
    ...
    <style>
        ...
        .server-selector { background: rgba(30, 41, 59, 0.8); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 12px; padding: 8px 12px; cursor: pointer; display: flex; align-items: center; gap: 8px; transition: all 0.3s; max-width: 250px; overflow: hidden; }
        .server-selector:hover { border-color: rgba(99, 102, 241, 0.5); background: rgba(30, 41, 59, 0.95); }
        .account-dropdown { position: relative; }
        .account-menu { position: absolute; top: 100%; right: 0; margin-top: 8px; background: rgba(30, 41, 59, 0.98); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 12px; padding: 8px; min-width: 200px; box-shadow: 0 10px 40px rgba(0, 0, 0, 0.5); z-index: 1000; }
        ...
        .stat-card { background: linear-gradient(135deg, rgba(99, 102, 241, 0.1), rgba(168, 85, 247, 0.1)); border: 1px solid rgba(99, 102, 241, 0.2); }
        .feature-highlight { background: linear-gradient(135deg, rgba(236, 72, 153, 0.1), rgba(168, 85, 247, 0.1)); }
                    ...
                    <div class="category-item" :class="activeCategory === cat.id ? 'active' : ''">
                        ...
                        <div x-show="expandedCategory === cat.id" class="submenu" x-transition>
                            <template x-for="cmd in cat.commands" :key="cmd.name">
                                <div class="submenu-item" @click="handleCommandClick(cmd.name)">
            ...
            <div class="flex items-center gap-3">
                ...
                <div class="account-dropdown" x-data="{ open: false }">
                    <button @click="open = !open" @click.away="open = false" class="server-selector">
                        <img :src="selectedGuild?.icon || 'https://cdn.discordapp.com/embed/avatars/0.png'" class="w-8 h-8 rounded-lg object-cover flex-shrink-0">
                        <span class="text-sm font-medium text-white truncate flex-1 min-w-0" x-text="selectedGuild?.name || 'Select Server'"></span>
                        <i class="fas fa-chevron-down text-xs text-slate-400 flex-shrink-0"></i>
                    </button>
                    <div x-show="open" 
                        ...
                        <template x-for="g in guilds" :key="g.id">
                            <button @click="selectGuild(g); open = false" class="account-menu-item">
                                <img :src="g.icon || 'https://cdn.discordapp.com/embed/avatars/0.png'" class="w-8 h-8 rounded-lg object-cover flex-shrink-0">
                                ...
                                <i class="fas fa-check text-emerald-400 flex-shrink-0" x-show="selectedGuild?.id === g.id"></i>
                            </button>
                        </template>
                    </div>
                </div>
    ...
    <script>
        function appData() {
            return {
                ...
                // NEW: Handle command clicks - special handling for view_plans
                handleCommandClick(cmdName) {
                    this.showMenu = false;
                    
                    // Special case: view_plans should open premium panel
                    if (cmdName === 'view_plans') {
                        this.activePanel = 'premium';
                        return;
                    }
                    
                    // For other commands, execute normally
                    this.executeCommand(cmdName);
                },
                
                async executeCommand(cmdName) { 
                    this.showMenu = false; 
                    ...
                    try { 
                        const res = await fetch('/api/command', { 
                            ...
                            body: JSON.stringify({ 
                                ...
                                command: cmdName.startsWith('!') ? cmdName : '!' + cmdName
                            }) 
                        }); 
                        const data = await res.json(); 
                        if (data.success) {
                            this.showToast(data.message || 'Command executed', 'success');
                        } else {
                            this.showToast(data.error || 'Command failed', 'error');
                        }
                    } catch (e) { 
                        this.showToast('Failed to execute command: ' + e.message, 'error'); 
                    } 
                },
...
def api_command():
    ...
    # Create Fake User with proper permissions
    class FakeUser:
        id = user_id
